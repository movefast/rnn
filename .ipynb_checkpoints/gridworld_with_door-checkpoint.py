...
class MazeEnvironment(BaseEnvironment):
    ...
    def env_start(self, keep_history=False):
        ...
        if self.keep_history:
            ...
            self.grid = mask_grid(self.grid, self.obstacles)
            self.grid = mask_grid(self.grid, *self.doors.values(), -1)
    ...
    def env_step(self, action):
        ...
        if tuple(self.current_state) in self.doors and self.doors[tuple(self.current_state)] in self.obstacles:
            self.obstacles.remove(self.doors[tuple(self.current_state)])
            self.door_open = True
            self.door_timer = 20
