...
class SimpleRNN(nn.Module):
    ...
    def batch(self, inp, hidden, discount_batch, action_batch):
        ...
        for i in range(inp.size(0)):
            ...
            # hidden *= 1 + self.actions[q_idx]
            if i > 0:
                states = torch.cat(hiddens)
                ws = F.softmax(hidden @ states.t() / torch.sqrt(states.norm(dim=1)))
                hidden = ws @ states
                # hidden = (hidden + ws @ states)
...
class RNNAgent(agent.BaseAgent):
    def agent_init(self, agent_init_info):
        ...
        self.state_buffer = ReplayMemory(10)
    ...
    def agent_start(self, state):
        ...
        self.state_buffer.append_state(self.hidden)
        with torch.no_grad():
    ...
    def agent_step(self, reward, state):    
        ...
        if len(self.state_buffer) < 10:
            states = torch.cat(list(self.state_buffer.memory))
            ws = F.softmax(self.hidden @ states.t() / torch.sqrt(states.norm(dim=1)))
            # self.hidden = (self.hidden + ws @ states) 
            self.hidden = ws @ states
        else:
            states = torch.cat(list(self.state_buffer.last_n(9)))
            ws = F.softmax(self.hidden @ states.t() / torch.sqrt(states.norm(dim=1)))
            # self.hidden = (self.hidden + ws @ states) 
            self.hidden = ws @ states
    ...
    def batch_train(self):
        self.train_steps += 1
        self.rnn.train()
        transitions = self.buffer.last_n(11)
        # batch = transitions[0]        
