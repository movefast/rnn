...
class SimpleRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, alpha, exp_decay):
    ...
    def batch(self, inp, hidden, discount_batch, action_batch):
        ...
        for i in range(inp.size(0)):
            ...
            hidden = (1 - self.alpha) * self.exp_decay * hidden + self.alpha * new_hidden
...
class RNNAgent(agent.BaseAgent):
    def agent_init(self, agent_init_info):
        ...
        self.rnn = SimpleRNN(self.num_states+1, self.num_states+1,self.num_actions, self.alpha, self.exp_decay).to(device)
        self.target_rnn = SimpleRNN(self.num_states+1, self.num_states+1,self.num_actions, self.alpha, self.exp_decay).to(device)
        self.update_target()
    ...
    def get_state_feature(self, state):
        ...
        is_door = torch.Tensor([float(is_door)]).to(device)
    ...
    def agent_start(self, state):
        ...
        with torch.no_grad():
            ...
            self.hidden = (1 - self.alpha) * self.exp_decay * self.hidden + self.alpha * new_hidden
