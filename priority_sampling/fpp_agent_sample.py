...
class RNNAgent(agent.BaseAgent):
    ...
    def batch_train(self):
        ...
        # q? why 1:
        # loss = torch.Tensor(is_weight[1:]).to(device) @ temp
        loss = torch.Tensor(is_weight[:-1]).to(device) @ temp
        errors = torch.abs((q_learning_action_values - target).squeeze_())
        for i in range(len(errors)):
            self.buffer.update(idxs[i], errors[i].item())
        loss += .5 *criterion_fpp(hidden_batch[1:], torch.cat(state_output))
        ...
        for i in range(hidden_batch.shape[0]):	
            state_updated = (hidden_batch[i:i+1] - self.step_size * is_weight[i] * hidden_batch.grad[i:i+1]).clone().detach()	
            # state_updated = (hidden_batch[i:i+1] - self.step_size * hidden_batch.grad[i:i+1]).clone().detach()	
            self.buffer.tree.update_value(idxs[i], transitions[i]._replace(hidden=state_updated))
