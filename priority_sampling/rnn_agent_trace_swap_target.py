...
class RNNAgent(agent.BaseAgent):
    ...
    def batch_train(self):
        ...
        current_q, hidden_batch = self.rnn.batch(state_batch, hidden_batch, discount_batch, action_batch)
        q_learning_action_values = current_q.max(1)[0]
        with torch.no_grad():
            new_q, _ = self.target_rnn.batch(next_state_batch[0:1], next_hidden_batch, next_discount_batch[0:1], next_action_batch[0:1])
        max_q = new_q.gather(1, next_action_batch[0:1])
        ...
        for i in range(len(max_q)):
            ...
            discounted_action_value_ests = discount_rates * q_learning_action_values[i:]
            # 1)
            ...
            # 4)
            lambdas = F.softmax(self.rnn.att(torch.cat(hidden_batch)).view(-1))
            target[i] += torch.sum(lambdas * temp)


        # episode_bound_mask = discount_rates > 0
        # discounted_returns = torch.zeros_like(reward_batch)
        # discounted_returns[episode_bound_mask] = torch.cumsum(discount_rates * reward_batch, 0)[episode_bound_mask]
        # discounted_returns += discount_rates * max_q




        # handle cross boundry case
        # if (discount_rates > 0).any().item():

        target = target.view(-1, 1)
        loss = criterion(target, max_q)
