...
class SimpleRNN(nn.Module):
    ...
    def batch(self, inp, hidden, discount_batch, action_batch, burn_in_len=0):
        output = []
        hiddens = []
        if len(inp.size()) == 2:
            inp = inp.unsqueeze(1)
...
class RNNAgent(agent.BaseAgent):
    ...
    def batch_train(self):
        ...
        current_q, state_outputs = self.rnn.batch(state_batch, hidden_batch, discount_batch, action_batch, self.burn_in_len)
