...
class RNNAgent(agent.BaseAgent):
    ...
    def agent_end(self, reward, state, append_buffer=True):
        ...
        if len(self.buffer) > 20:
            self.batch_train()
        evaluation.metrics.rep_loss['RNN_Zero'].append(self.episodic_metrics)
        self.episodic_metrics = []
    ...
    def batch_train(self):
        ...
        next_hidden_batch = torch.zeros_like(batch.hidden[1])
        ...
        current_q, state_outputs = self.rnn.batch(state_batch, hidden_batch, discount_batch, action_batch)
        ...
        with torch.no_grad():
            ...
            new_q, _ = self.rnn(batch.state[-1], state_outputs[-1])
    ...
    # def calc_q_discrepency(self, old_hidden, new_hidden, all_qs):
    #     with torch.no_grad():
    #         old_q, _ = self.rnn(state, old_hidden)
    #         new_q, _ = self.rnn(state, new_hidden)
    #     return rep_crit(new_q, old_q) / all_qs.abs().max()
