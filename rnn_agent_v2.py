...
class RNNAgent(agent.BaseAgent):
    def agent_init(self, agent_init_info):
        ...
        self.episodic_buffer = ReplayMemory(20)
        self.tau = .5
        self.flag = False
        self.train_steps = 0    
    ...
    def agent_step(self, reward, state):    
        ...
        self.episodic_buffer.push(self.prev_state, self.prev_action, reward, self.hidden.detach(), self.discount)
        ...
        if self.steps % 10 == 0 and len(self.episodic_buffer) == 20:
            self.batch_add_to_buffer()

        if len(self.buffer) > 10:# and self.steps % 5 == 0:# and self.epsilon == .1:
            self.batch_train()
        return action
    ...
    def agent_end(self, reward, state, append_buffer=True):
        ...
        state = self.get_state_feature(state)
        if append_buffer:
            self.episodic_buffer.push(self.prev_state, self.prev_action, reward, self.hidden.detach(), 0)
            self.batch_add_to_buffer()
        self.episodic_buffer.clear()
        if len(self.buffer) > 10:
            self.batch_train()

    def batch_add_to_buffer(self):
        batch = Transition(*zip(*self.episodic_buffer.memory))
        self.buffer.push(batch.state, batch.action, batch.reward, batch.hidden, batch.discount)
