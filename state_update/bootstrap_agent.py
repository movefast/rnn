...
class RNNAgent(agent.BaseAgent):
    def agent_init(self, agent_init_info):
        ...
        self.beta = agent_init_info.get("beta", 1)
    ...
    def batch_train(self):
        ...
        with torch.no_grad():   
            new_q, target_state_output = self.target_rnn.batch(next_state_batch, next_hidden_batch, next_discount_batch, next_action_batch)
        max_q = new_q.max(1)[0]
        target = reward_batch
        ...
        loss += self.beta * criterion_fpp(state_output[-1], target_state_output[-2])
